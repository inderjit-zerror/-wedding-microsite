...
export default function FAQ() {
  ...
  return (
    <section
      id="FAQ"
      className="min-h-screen max-sm:min-h-fit COLOR_BG_RED text-white flex items-center justify-center max-sm:py-[20vw]"
    >
      <div className="w-[80%] max-sm:w-full px-6 max-sm:px-[20px] flex flex-col gap-20">
        ...
        <div className="space-y-6 w-full">
          {faqs.map((question, index) => (
            <div key={index} className="border-t border-white/20 pt-6  group translate-0">
              ...
              <div className='w-0 h-px bg-white group-hover:w-full absolute bottom-[-50%] left-0  transition-all duration-300 ease-in'></div>
            </div>
          ))}
